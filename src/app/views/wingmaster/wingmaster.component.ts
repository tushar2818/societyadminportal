...
export class WingMasterComponent {
  ...
  uniqueKey: any = "WingMasterID";
  pageTitleKey: any = "PageTitles.WingMaster";
  ...
  //constructor 
  constructor(
    private service: WingMasterService,
  ...
  ngOnInit() {
    try {
      this.modelHeaders = ["TableHeaders.WingName", "TableHeaders.NoOfFloor"];
  ...
  //refresh buildings
  refreshBuildings() {
    let allLookupKeys = [{ LookupType: LookupType.AllBuildings }];
    this.subscription = this.globalService.getallLookups(allLookupKeys).subscribe(response => {
      try {
        if (response != null && response.IsSuccess && response.Result != null) {
          let result = response.Result;
          this.buildingList = result.filter(s => s.LookupType == LookupType.AllBuildings)[0].Data;
          if (this.buildingList != null && this.buildingList.length > 0) {
            this.selectedBuildingId = this.buildingList[0].BuildingMasterID;
          }
          this.refresh();
        }
        else {
  ...
  refresh() {
    ...
    this.subscription = this.service.getall(this.selectedBuildingId).subscribe(response => {
      try {
  ...
  onSave(isValid: boolean) {
    if (isValid) {
      this.model.BuildingMasterID = this.selectedBuildingId;
      this.globalService.showLoading(true);
